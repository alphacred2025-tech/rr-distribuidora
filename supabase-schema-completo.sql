-- ================================================================
-- RR Distribuidora — SCHEMA COMPLETO v3
-- Cole no SQL Editor do Supabase e execute
-- Apaga tudo e recria do zero com tudo correto
-- ================================================================
...
-- ================================================================
-- 2. FUNÇÕES SEM DEPENDÊNCIA (criadas antes das tabelas que as usam)
-- ================================================================
...
-- Entregas (UNIQUE em pedido_id = one-to-one com pedidos)
...
-- Produtos: cestas (o que vende) + insumos (o que compra em fardo)
CREATE TABLE public.produtos (
  ...
  categoria      TEXT NOT NULL DEFAULT 'insumo'
                   CHECK (categoria IN ('cesta','insumo','embalagem','outro')),
  ativo          BOOLEAN NOT NULL DEFAULT TRUE,
  imagem_url     TEXT,
  criado_em      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  atualizado_em  TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
-- ================================================================
-- 5. FUNÇÃO RPC — Desconta ingredientes do estoque (anon pode chamar)
--    SECURITY DEFINER = roda com permissão elevada, mas é segura
-- ================================================================
...
-- profiles
CREATE POLICY "own_profile"           ON public.profiles FOR SELECT TO authenticated USING (id = auth.uid() OR meu_papel() = 'admin');
CREATE POLICY "admin_update_profiles" ON public.profiles FOR UPDATE TO authenticated USING (meu_papel() = 'admin');
CREATE POLICY "admin_insert_profiles" ON public.profiles FOR INSERT TO authenticated WITH CHECK (meu_papel() = 'admin');
...
CREATE POLICY "auth_update_clientes"  ON public.clientes FOR UPDATE TO authenticated USING (TRUE);

-- pedidos
CREATE POLICY "anon_insert_pedidos"   ON public.pedidos  FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "auth_select_pedidos"   ON public.pedidos  FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "auth_update_pedidos"   ON public.pedidos  FOR UPDATE TO authenticated USING (meu_papel() IN ('admin','logistica','vendedor'));

-- itens_pedido
CREATE POLICY "anon_insert_itens"     ON public.itens_pedido FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "auth_select_itens"     ON public.itens_pedido FOR SELECT TO authenticated USING (TRUE);

-- entregas
CREATE POLICY "anon_insert_entregas"  ON public.entregas FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "auth_insert_entregas"  ON public.entregas FOR INSERT TO authenticated WITH CHECK (meu_papel() IN ('admin','logistica','montador'));
CREATE POLICY "auth_select_entregas"  ON public.entregas FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "auth_update_entregas"  ON public.entregas FOR UPDATE TO authenticated USING (meu_papel() IN ('admin','logistica','montador','motoboy'));

-- lancamentos
CREATE POLICY "anon_insert_lancamentos"  ON public.lancamentos FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "auth_select_lancamentos"  ON public.lancamentos FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "auth_insert_lancamentos"  ON public.lancamentos FOR INSERT TO authenticated WITH CHECK (meu_papel() IN ('admin','financeiro'));
CREATE POLICY "auth_update_lancamentos"  ON public.lancamentos FOR UPDATE TO authenticated USING (meu_papel() IN ('admin','financeiro'));
CREATE POLICY "admin_delete_lancamentos" ON public.lancamentos FOR DELETE TO authenticated USING (meu_papel() = 'admin');

-- configuracoes
CREATE POLICY "auth_read_config"      ON public.configuracoes FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "admin_write_config"    ON public.configuracoes FOR ALL    TO authenticated USING (meu_papel() = 'admin');

-- motoboys
CREATE POLICY "auth_read_motoboys"    ON public.motoboys FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "admin_write_motoboys"  ON public.motoboys FOR ALL    TO authenticated USING (meu_papel() IN ('admin','logistica'));

-- metas_vendas
CREATE POLICY "auth_read_metas"       ON public.metas_vendas FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "admin_write_metas"     ON public.metas_vendas FOR ALL    TO authenticated USING (meu_papel() = 'admin');

-- produtos (anon pode ver ingredientes ativos para exibição no site)
CREATE POLICY "anon_select_produtos"  ON public.produtos FOR SELECT TO anon          USING (ativo = TRUE);
CREATE POLICY "auth_select_produtos"  ON public.produtos FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "auth_write_produtos"   ON public.produtos FOR ALL    TO authenticated USING (meu_papel() IN ('admin','logistica')) WITH CHECK (meu_papel() IN ('admin','logistica'));

-- estoque_movimentos
CREATE POLICY "auth_select_movimentos" ON public.estoque_movimentos FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "anon_insert_movimentos" ON public.estoque_movimentos FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "auth_insert_movimentos" ON public.estoque_movimentos FOR INSERT TO authenticated WITH CHECK (meu_papel() IN ('admin','logistica'));

-- afiliados
CREATE POLICY "anon_select_afiliados"  ON public.afiliados FOR SELECT TO anon          USING (ativo = TRUE);
CREATE POLICY "auth_select_afiliados"  ON public.afiliados FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "admin_write_afiliados"  ON public.afiliados FOR ALL    TO authenticated USING (meu_papel() = 'admin') WITH CHECK (meu_papel() = 'admin');

-- comissoes_afiliados
CREATE POLICY "auth_select_comissoes"  ON public.comissoes_afiliados FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "anon_insert_comissoes"  ON public.comissoes_afiliados FOR INSERT TO anon          WITH CHECK (TRUE);
CREATE POLICY "admin_update_comissoes" ON public.comissoes_afiliados FOR UPDATE TO authenticated USING (meu_papel() = 'admin');
...
-- ================================================================
-- 10. DADOS INICIAIS — Cestas (produtos vendidos) + Insumos (comprados em fardo)
-- ================================================================

-- CESTAS: o que a RR vende para o cliente
...
-- INSUMOS: o que a RR compra em fardo para montar as cestas
-- custo = preço por unidade (calculado com base no preço do fardo)
INSERT INTO public.produtos (nome, descricao, preco, custo, estoque_atual, estoque_minimo, unidade, categoria) VALUES
  ('Arroz 5kg',                  'Arroz branco 5kg — comprado em fardo 30un',          22.00, 18.50, 200, 50, 'un', 'insumo'),
  ('Feijão 1kg',                 'Feijão carioca 1kg — comprado em fardo 30un',        10.00,  7.80, 200, 50, 'un', 'insumo'),
  ('Açúcar 2kg',                 'Açúcar cristal 2kg — comprado em fardo 20un',         9.00,  7.00, 200, 50, 'un', 'insumo'),
  ('Sal 1kg',                    'Sal refinado 1kg — comprado em fardo 30un',            3.50,  2.50, 200, 50, 'un', 'insumo'),
  ('Óleo de Soja 900ml',         'Óleo de soja 900ml — comprado em fardo 20un',        10.00,  8.20, 200, 50, 'un', 'insumo'),
  ('Farinha de Trigo 1kg',       'Farinha de trigo 1kg — comprado em fardo 25un',       5.00,  3.80, 150, 40, 'un', 'insumo'),
  ('Macarrão 500g',              'Macarrão espaguete 500g — comprado em fardo 30un',    4.50,  3.20, 150, 40, 'un', 'insumo'),
  ('Molho de Tomate 340g',       'Molho de tomate 340g — comprado em fardo 24un',       4.00,  2.90, 150, 40, 'un', 'insumo'),
  ('Sardinha 125g',              'Sardinha em óleo 125g — comprado em fardo 48un',      6.50,  5.00, 100, 30, 'un', 'insumo'),
  ('Café 250g',                  'Café torrado e moído 250g — comprado em fardo 30un',  9.00,  7.00, 100, 30, 'un', 'insumo'),
  ('Biscoito Cream Cracker 400g','Biscoito cream cracker 400g — fardo 20un',            6.00,  4.50, 100, 30, 'un', 'insumo'),
  ('Leite em Pó 400g',           'Leite em pó 400g — comprado em fardo 12un',         18.00, 14.00, 100, 30, 'un', 'insumo'),
  ('Achocolatado 400g',          'Achocolatado em pó 400g — comprado em fardo 12un',  14.00, 10.50, 100, 30, 'un', 'insumo'),
  ('Sabão em Pó 1kg',            'Sabão em pó 1kg — comprado em fardo 20un',          12.00,  9.00, 100, 30, 'un', 'insumo'),
  ('Amaciante 500ml',            'Amaciante 500ml — comprado em fardo 20un',           8.00,  6.00, 100, 30, 'un', 'insumo'),
  ('Detergente 500ml',           'Detergente 500ml — comprado em fardo 24un',          3.50,  2.50, 100, 30, 'un', 'insumo'),
  ('Papel Higiênico 4un',        'Papel higiênico 4 rolos — comprado em fardo 16cx',   8.00,  6.00, 100, 30, 'un', 'insumo'),
  ('Creme Dental',               'Creme dental 90g — comprado em fardo 48un',          4.50,  3.20,  80, 20, 'un', 'insumo'),
  ('Sabonete',                   'Sabonete 90g — comprado em fardo 72un',              3.00,  2.00,  80, 20, 'un', 'insumo')
ON CONFLICT DO NOTHING;

-- ================================================================
-- 11. ADMIN PADRÃO — cria/atualiza o usuário admin
-- ================================================================
...
-- ================================================================
-- FIM DO SCHEMA — RR Distribuidora v3
-- ================================================================
